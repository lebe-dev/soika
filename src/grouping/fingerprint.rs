//! Default Sentry-style fingerprint, issue title, and culprit derivation.
//!
//! A custom `fingerprint` from the SDK overrides the default; the default
//! derives from exception type + a normalized in-app stacktrace.
